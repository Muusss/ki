    ...
    <div class="grid lg:grid-cols-3 gap-8">
          ...
          <form action="{{ route('public.permintaan.store') }}" method="POST" class="space-y-4">
            ...
            <div>
              ...
              <textarea name="komposisi" rows="3" required
                        class="w-full px-4 py-3 rounded-2xl border border-pink-200 focus:border-pink-400 focus:outline-none focus:ring-2 focus:ring-pink-200 transition-all"
                        placeholder="Tuliskan kandungan utama produk">{{ old('komposisi') }}</textarea>
              @error('komposisi')
                <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
              @enderror
            </div>
            ...
            <div>
              ...
              <select name="harga" required
                ...
                <option value="<50k"   {{ old('harga') === '<50k' ? 'selected' : '' }}>< Rp 50.000</option>
                <option value="50-100k"{{ old('harga') === '50-100k' ? 'selected' : '' }}>Rp 50.000 - 100.000</option>
                <option value=">100k"  {{ old('harga') === '>100k' ? 'selected' : '' }}>> Rp 100.000</option>
              </select>
              @error('harga')
                <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
              @enderror
            ...
            <div>
              ...
              <select name="spf" required
                ...
                <option value="30"  {{ old('spf') === '30'  ? 'selected' : '' }}>SPF 30</option>
                <option value="35"  {{ old('spf') === '35'  ? 'selected' : '' }}>SPF 35</option>
                <option value="40"  {{ old('spf') === '40'  ? 'selected' : '' }}>SPF 40</option>
                <option value="50+" {{ old('spf') === '50+' ? 'selected' : '' }}>SPF 50+</option>
              </select>
              @error('spf')
                <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
              @enderror
            ...
            <!-- Submit Button -->
            <button type="submit"
                    class="w-full bg-gradient-to-r from-pink-500 to-pink-400 text-white py-3 rounded-2xl hover:shadow-lg transform hover:scale-105 transition-all duration-300 font-semibold">
              Kirim Permintaan
            </button>
          </form>
      ...
      <div class="lg:col-span-2" data-aos="fade-left">
                        ...
                        @elseif($item->status === 'rejected')
                          <div class="inline-flex flex-col items-center">
                            ...
                            @if(!empty($item->admin_notes))
                              <span class="mt-1 text-[12px] text-red-500">
                                Alasan: {{ $item->admin_notes }}
                              </span>
                            @endif
                          </div>
                        @else
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
          <div class="glass rounded-2xl p-4 text-center" data-aos="zoom-in" data-aos-delay="50">
            <h3 class="text-2xl font-bold text-pink-500">{{ $total }}</h3>
          ...
          <div class="glass rounded-2xl p-4 text-center" data-aos="zoom-in" data-aos-delay="100">
            <h3 class="text-2xl font-bold text-green-500">{{ $approved }}</h3>
            <p class="text-sm text-gray-600">Disetujui</p>
          </div>
          <div class="glass rounded-2xl p-4 text-center" data-aos="zoom-in" data-aos-delay="150">
            <h3 class="text-2xl font-bold text-orange-500">{{ $pending }}</h3>
            <p class="text-sm text-gray-600">Menunggu</p>
          </div>
          <div class="glass rounded-2xl p-4 text-center" data-aos="zoom-in" data-aos-delay="200">
            <h3 class="text-2xl font-bold text-red-500">{{ $rejected }}</h3>
